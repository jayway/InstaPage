...
class InstaPageViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    public lazy var pages: [UIViewController] = {
        return [
            self.getViewController(color: #colorLiteral(red: 0.9749749303, green: 1, blue: 0.437482059, alpha: 1), numberOfRows: 50),
            self.getViewController(color: #colorLiteral(red: 0.7227508426, green: 1, blue: 0.9128507376, alpha: 1), numberOfRows: 10),
            self.getViewController(color: #colorLiteral(red: 1, green: 0.7548497319, blue: 1, alpha: 1), numberOfRows: 90)
        ]
    }()
    ...
    override func viewDidLoad() {
        ...
        {
            self.setViewControllers([firstVC], direction: .forward, animated: true) { (completed) in
                if let vc = firstVC as? InstaContentViewController{
                    self.tableViewDelegate.updateBottomConstraint(for: vc.tableView)
                }
            }
        }
    ...
    fileprivate func getViewController(color: UIColor, numberOfRows: Int) -> UIViewController
    {
